...
def attach_query_seq(bam_file, fasta_file, out_file):
    ...
    for idx, read in enumerate(bam.fetch()):
        ...
        cols = '\t'.join(map(str, [idx, read.reference_name, read.reference_start, read.reference_end, read.query_name, read.query_alignment_start, read.query_alignment_end, strand, "60"]))
        output = cols+"\n"+query_seq+"\n"+ref_seq+"\n"
        out.write(output)
    out.close()
    bam.close()
