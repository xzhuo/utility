...
RMlib = sys.argv[1]
dict5 = {}  # the dictionary to store seq.id and seq from repeatmasker library
dict3 = {}
dictorf = {}
fh = open(RMlib, "r")
for rec in SeqIO.parse(fh, "embl"):
    if re.match(r'LINE', rec.annotations["keywords"][0]):
        # if it is a LINE:
        if rec.name[-5:] == '_5end':
            # got a 5'end LINE:
            dict5[rec.name] = rec
