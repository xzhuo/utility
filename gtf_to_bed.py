...
def print_to_bed():
    ...
    for num, exon_int in enumerate(exon_list):
        exon.write("%s\t%d\t%d\t%s\t.\t%s\n" % (curr_chrom, exon_int[0], exon_int[1], curr_transcript, curr_strand))
        if num:  # if num > 0, means it is at least 2nd exon.
            intron.write("%s\t%d\t%d\t%s\t.\t%s\n" % (curr_chrom, intron_start, exon_int[0]-1, curr_transcript, curr_strand))
