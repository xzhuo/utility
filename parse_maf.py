        ...
        with open(args.out, 'w') as Out:
            for num, line in enumerate(Fh, 1):
                ...
                if lead == 'a':  # new alignmentblock
                    ...
                    #     # pseudo codes first:
                    #     blockgap, merged_block = compare_merge_blocks(last_block, curr_block, genomes, args.gap)
                    #     # blockgap could be:
                    #     # merge (both block with same "aln" status and indel < args.gap);
                    #     # hold (two blocks with different "aln" status and indel < args.gap);
                    #     # break (two blocks with indel > args.gap)
                    #     if blockgap == 'merge' or blockgap == 'hold':
                    #         last_block = copy.deepcopy(merged_block)
                    #     # elif blockgap == 'hold':
                    #     #     holdingblockgap, merged_holdingblock = compare_merge_blocks(holding_block, last_block, genomes, args.gap)
                    #     #     if holdingblockgap == 'merge' or holdingblockgap == 'hold':
                    #     #         holding_block = copy.deepcopy(merged_holdingblock)
                    #     #     else:
                    #     #         print_block(holding_block, Out)
                    #     #         holding_block = copy.deepcopy(last_block)
                    #     #     last_block = copy.deepcopy(curr_block)
                    #     else:  # means it is a 'break'
                    #         holdingblockgap, merged_holdingblock = compare_merge_blocks(holding_block, last_block, genomes, args.gap)
                    #         if holdingblockgap == 'merge' or holdingblockgap == 'hold':
                    #             holding_block = merged_holdingblock
                    #             print_block(holding_block, Out)
                    #         else:
                    #             print_block(holding_block, Out)
                    ...
                    if curr_block:
                        ...
                        # Two_blocks, merged_block = compare_merge_blocks(last_block, curr_block, genomes, args.gap)
                        # # if Two_blocks == 'new':
                        # #     last_block = copy.deepcopy(curr_block)
                        # if Two_blocks == "aln":
                        #     last_block = copy.deepcopy(merged_block)
                        ...
                        #     last_block = copy.deepcopy(merged_block)
                        # else:
                        #     Two_full_blocks, merged_fullblock = compare_merge_blocks(holding_block, last_block, genomes, args.gap)
                        #     if Two_full_blocks == "break":
                        #         last_fullblock = copy.deepcopy(merged_fullblock)
                        #     else:
                        #         if _is_complete(last_fullblock, genomes):
                        #             print_block(last_fullblock, Out)
                        #         last_fullblock = copy.deepcopy(last_block)
                        #     # if _is_complete(curr_block, genomes):
                        #     last_block = copy.deepcopy(curr_block)
                ...
                #     last_block = copy.deepcopy(merged_block)
                # else:
                #     Two_full_blocks, merged_fullblock = compare_merge_blocks(last_fullblock, last_block, genomes, args.gap)
                #     if Two_full_blocks == "break":
                #         last_fullblock = copy.deepcopy(merged_fullblock)
                #     else:
                ...
                #             print_block(last_block, Out)
                    ...
                    #     if holdingblockgap == 'merge' or holdingblockgap == 'hold':
                    #         holding_block = copy.deepcopy(merged_holdingblock)
                    #         blockgap, merged_block = compare_merge_blocks(holding_block, curr_block, genomes, args.gap)
                    #         if blockgap == 'merge' or blockgap == 'hold':
                    #             print_block(merged_block, Out)
                    #         else:
                    #             print_block(holding_block, Out)
                ...
                #     Two_full_blocks, merged_fullblock = compare_merge_blocks(last_fullblock, last_block, genomes, args.gap)
                #     if Two_full_blocks == "break":
                #         if _is_complete(merged_fullblock):
                #             print_block(merged_fullblock)
                #     else:
                #         if _is_complete(last_fullblock):
                #             print_block(last_fullblock)
                #         if _is_complete(last_block):
